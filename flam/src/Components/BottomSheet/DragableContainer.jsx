...
function DraggableContainer (props) {
    const {handleSheetClose, onSheetDragged, headerText, content, handleOpacity} = props
  ...
  return (
    ...
    >
      <div className={styles.sheetContentContainer}> 
        ...
        <div style={{ height: `${contentHeight}px` }} className={styles.sheetContentWrapper}>
            <div style={{paddingLeft: '50px', paddingRight: '50px'}}>
                {content()}
            </div>
        </div>
        <div className={styles.footer}>
            <button onClick={handleSheetClose} className={styles.doneButton}>Done</button>
        </div>
      </div>
    </div>
  );
